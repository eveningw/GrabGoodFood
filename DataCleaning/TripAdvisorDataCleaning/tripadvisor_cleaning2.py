...
def update_opening_hours(collection):
    # 定義查詢條件
    query = {'hours/weekRanges/0/0/openHours': {'$exists': True, '$ne': None}}
    
    # 定義更新操作
    update = {
        '$set': {
            'weekday': 0,
            'open_time': "$hours/weekRanges/0/0/openHours"
        }
    }

    # 使用 update_many 並指定 aggregation pipeline
    result = collection.aggregate(query, update)
...
def main():
    # 連接到 MongoDB
    uri = 'mongodb://localhost:27017/'  # 替換為您的 MongoDB 連接字串
    db_name = 'tripadvisor'
    collection_name = 'restaurants1'
    ...
    # 呼叫函數更新資料
    # modified_count = update_opening_hours(collection)
