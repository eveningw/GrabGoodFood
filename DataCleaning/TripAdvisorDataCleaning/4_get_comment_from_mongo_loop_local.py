...
def extract_comments_from_mongodb(db_name: str, collection_name: str, skip: int, limit: int):
    try:
        # 連接到 MongoDB
        ...
        # 提取指定範圍的資料
        for restaurant in collection.find().skip(skip).limit(limit):
            ...
            if isinstance(comments, list):
                ...
                for item in comments:
                    ...
                    if comment_date is not None and comment_date > one_year_before:
...
start_index = 4
limit_per_extraction = 2
total_extractions = 540  # 總共提取的次數

for extraction in range(total_extractions):
    skip = start_index + (extraction * limit_per_extraction)

    # 獲取評論資料並生成 user_input
    user_input = extract_comments_from_mongodb(db_name, collection_name, skip=skip, limit=limit_per_extraction)
...
"""
    ...
    if all_responses:
        ...
        file_name = f"google_Comment_0304.csv"   

        # 將資料附加到現有的 CSV 檔案中
        df.to_csv( file_name, mode='a', index=False, header=not os.path.exists(file_name), encoding='utf-8-sig')
        print(f"第 {extraction + 1} 次提取的數據已成功寫入 google_Comment_0304.csv")
    else:
        print("沒有有效的回應可寫入文件。")
